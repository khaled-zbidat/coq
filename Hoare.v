...
Theorem hoare_asgn_fwd :
  ...
  {{fun st => P (X !-> m ; st)
           /\ st X = aeval (X !-> m ; st) a }}.
Proof.
 unfold hoare_triple. split. 
  - inversion H0. subst. inversion H. subst. 
    replace (t_update (t_update st X (aeval st a)) X (st X)) with st.
    auto.
    symmetry. rewrite t_update_shadow. rewrite t_update_same. auto.
  - inversion H0. subst. inversion H. subst.   
...
Proof.
  ...
  intros st H. unfold assn_sub. simpl. split. compute. apply H. apply t_update_eq.
  eapply hoare_consequence_pre. apply hoare_asgn.
  intros st H.  unfold assn_sub. auto.
...
specialize H with (a := <{X}>) (n := 0).
  assert((X!->0) =[X := 3 ; Y:= X]=> (Y !-> 3;(X !-> 3; X !-> 0))).
  + apply E_Seq with (X !-> 3; X !-> 0).
    - eapply E_Ass. simpl. auto.
    - eapply E_Ass. simpl. auto.
  + apply H in H0.
...
i will prove this by induction, for the base action:- we apply the sequince  x = 1, y =x and x = x-1
so we get y=1 and x = 0, We check till we get to the cond and then we stop the loop and we use E_ReapetTrue
app.

teh induc assump:- if X=n the process satisfies pre and post conditions and We'll prove it for X=n+1.
 the proof:
let x be x = n +1 , in the first iteration we y will be eq to n + 1 and x will be x = n and we will apply
E_RepeatFalse were x = n we will still repeate it till we get where x will be equal to 0 and y will be 
greater than 0;
...
Proof.
  ...
  - apply hoare_havoc.
  - unfold "->>".  unfold havoc_pre. intros. unfold assn_sub. simpl.
    exists (aeval st X). rewrite t_update_shadow.  rewrite t_update_same. simpl. apply H.

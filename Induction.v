...
(** **** Exercise: 2 star s, standard, especially useful (basic_induction) 
...
Proof.
  ...
  simpl.  reflexivity.
  rewrite h. rewrite negb_involutive. simpl. reflexivity.
Qed. 
...
Proof.
  ...
  reflexivity.
   ...
   assert (g: m + S n' =S n' + m). {rewrite -> plus_comm. reflexivity. }
...
Proof.
  ...
  simpl. rewrite -> h. rewrite -> mult_comm_helper. reflexivity.  
...
Proof.
  ...
  reflexivity.
    rewrite plus_assoc.   rewrite plus_assoc.
   replace  (m + S n') with (S n' + m). reflexivity.
    {rewrite -> plus_comm. reflexivity. }
Qed.

...
(** **** Exercise: 1 star, standard, optional (ev_double)  *)
...
(** **** Exercise: 1 star, standard, optional (inversion_practice) 
...
(** **** Exercise: 1 star, standard, optional (ev5_nonsense) 
...
(** **** Exercise: 2 stars, standard, optional (ev_sum)  *)
...
(** **** Exercise: 3 stars, advanced, optional (ev_ev__ev) 
...
(** **** Exercise: 2 stars, advanced, optional (subsequence) 
...
(** **** Exercise: 3 stars, standard, optional (exp_match_ex1) 
...
(** **** Exercise: 5 stars, standard (weak_pumping) 
...
Proof.
  ...
  - (* MEmpty *)
    simpl. lia.
  - simpl. lia. 
  - simpl.  simpl. rewrite app_length.  intros H. apply add_le_cases in H. destruct H as [H1 | H2].
    + apply IH1 in H1. destruct H1 as [s2' [s3' [s4' [H0 [H1 H2]]]]]. exists s2'. exists s3'. 
      exists (s4'++s2). split.
      *  symmetry. rewrite H0. replace (s2' ++ s3' ++ s4' ++ s2) with  ((s2' ++ s3' ++ s4') ++ s2). auto. auto.  rewrite <- app_assoc. rewrite <- app_assoc.  auto.
      * split. auto.   intros. auto.
       replace (s2' ++ napp m s3' ++ s4' ++ s2) with ((s2' ++ napp m s3' ++ s4') ++ s2).
          apply (MApp (s2' ++ napp m s3' ++ s4')). auto. auto. auto. 
        symmetry.  replace (s2' ++ s3' ++ s4' ++ s2) with  ((s2' ++ s3' ++ s4') ++ s2).   rewrite <- app_assoc. rewrite <- app_assoc.  auto. rewrite <- app_assoc. rewrite <- app_assoc. auto.
    + intros. apply IH2 in H2. inversion H2 as [a [ b [ c[H1'[H2' H3']]]]]. exists (s1 ++ a). exists b. exists c. split.
        * rewrite H1'. symmetry. rewrite <- app_assoc. reflexivity.
        * split. apply H2'. intros m.  rewrite <- app_assoc. apply MApp. auto. auto. 
  - auto. simpl. intros H.   apply plus_le in H . destruct H. apply IH in H.  inversion H as [a [ b [ c[H1'[H2' H3']]]]].
    exists a. exists b. exists c.
    split. +  apply H1'. + split. apply H2'. intros . apply MUnionL. auto.
  - auto. simpl. intros H. apply plus_le in H . destruct H. + apply IH in H0. inversion H0 as [a [ b [ c[H1'[H2' H3']]]]].
    exists a. exists b. exists c.  split. apply H1'. split.  apply H2'. intros m. apply MUnionR. auto.
  - simpl. intros H. simpl. inversion H. apply pumping_constant_0_false in H1. auto. inversion H1.
  - simpl. intros. rewrite app_length in H.
    destruct s1.
    + destruct s2. * inversion H. simpl in H. simpl in IH2. apply IH2 in H. destruct H as [s1 [s3 [s4 [H1' [H2' H3']]]]].
         exists s1, s3, s4. split. rewrite <- H1'. reflexivity. split. apply H2'. apply H3'.
          * simpl in H. simpl in IH2. apply IH2 in H. destruct H as [s1 [s3 [s4 [H1 [H2 H3]]]]].
          rewrite H1. simpl. exists s1, s3, s4. split. reflexivity. split. apply H2. apply H3.
    + exists [], (x :: s1), s2. split. * reflexivity. 
      * split. intro. discriminate. { intro. simpl. apply star_app. - induction m. + simpl. apply MStar0.
            + remember (x :: s1) as s1'. simpl. apply star_app. rewrite <- app_nil_r with (l := s1'). auto.
              apply MStarApp. auto. auto.  apply MStar0. apply IHm. 
  - auto. } 
Qed.

(** [] *)

(** **** Exercise: 5 stars, standard (pumping) 
...
Lemma add_le_cases' : forall n m p q :nat, n+m <= p + q -> (n<=p /\ m<=q)\/ (n>p /\ m<=q) \/(n<=p /\ m>q).
Proof.
intros. lia.
Qed.
...
Lemma MStar_plus :
  forall T s1 s2 (re : reg_exp T),
     ...
     s1 ++ s2 =~ Star re.
Proof.
  intros. apply star_app. apply MStar1. 
  - apply H.
  - apply H0.
Qed.
...
Lemma le_n_0_eq : forall s, s <= 0 -> s=0.
Proof.
...
Proof.
  ...
  - (* MApp *)     
    ...
    apply add_le_cases' in H. destruct H.
        ...
        + destruct IH1 as [s2' [s3' [s4' [H0 [H1 H2]]]]]. apply H. exists s2'. exists s3'. 
      exists (s4'++s2). split.
      *  symmetry. rewrite H0. replace (s2' ++ s3' ++ s4' ++ s2) with  ((s2' ++ s3' ++ s4') ++ s2). auto. auto.  rewrite <- app_assoc. rewrite <- app_assoc.  auto.
      * split. auto. split. assert (pumping_constant re1 <= pumping_constant re1 + pumping_constant re2).
        apply le_plus_l.  apply le_trans with (n:=pumping_constant re1). apply H2.
        apply le_plus_l.
   ...
   intros. auto.
       ...
       replace (s2' ++ napp m s3' ++ s4' ++ s2) with ((s2' ++ napp m s3' ++ s4') ++ s2).
          apply (MApp (s2' ++ napp m s3' ++ s4')). apply H2. auto. auto. 
        symmetry.  replace (s2' ++ s3' ++ s4' ++ s2) with  ((s2' ++ s3' ++ s4') ++ s2).   rewrite <- app_assoc. rewrite <- app_assoc.  auto. rewrite <- app_assoc. rewrite <- app_assoc. auto.
    + destruct H. 
        ** intros. destruct IH2 as [a [ b [ c[H1'[H2' H3']]]]]. apply H. exists (s1 ++ a). exists b. exists c. split.
        * rewrite H1'. symmetry. rewrite <- app_assoc. reflexivity.
        * split. apply H2'. split. 

            apply le_trans with (n:=length s1 + pumping_constant re2).
            assert(length (s1 ++ a) = length s1 + length a). apply app_length. lia. apply Plus.plus_le_compat_r. lia.
            intros m.  rewrite <- app_assoc. apply MApp. auto. destruct H3'. apply H1. 
        ** intros. destruct IH1 as [x [ x0 [ x1[H1'[H2' H3']]]]].
            apply H. (inversion H3'). exists x, x0, (x1++s2).
            split. rewrite H1'. rewrite <- app_assoc. rewrite <- app_assoc. auto. split. auto. split.  apply le_trans with (n:=pumping_constant re1). apply H0.
            apply le_plus_l.
             
            intro. rewrite -> app_assoc. rewrite -> app_assoc. 
            assert (((x ++ napp m x0) ++ x1) =~ re1). rewrite <- app_assoc. apply H1. 
            simpl. apply MApp . apply H2. apply Hmatch2.
  - auto. simpl. intros H.   apply plus_le in H . destruct H. apply IH in H.  inversion H as [a [ b [ c[H1'[H2' H3']]]]].
    exists a. exists b. exists c.
    split. +  apply H1'. + split. apply H2'. split. inversion H3'. assert(pumping_constant re1 <= pumping_constant re1 + pumping_constant re2). lia.
    apply le_trans with (n:=pumping_constant re1). auto. lia. 
    intros . apply MUnionL. apply H3'.
  - auto. simpl. intros H. apply plus_le in H . destruct H. + apply IH in H0. inversion H0 as [a [ b [ c[H1'[H2' H3']]]]].
    exists a. exists b. exists c.  split. apply H1'. split.  apply H2'. split. lia.    intros m. apply MUnionR. auto. apply H3'.
  - simpl. intros H. simpl. inversion H. apply pumping_constant_0_false in H1. auto. inversion H1.
  - intros. assert(H':= H). simpl in H. assert (pumping_constant re <= length (s1 ++ s2) -> pumping_constant re + length s1 <= length (s1 ++ s2) + length s1). lia.
  apply H0 in H.   rewrite app_length in H. clear H0. 
  assert(pumping_constant re + length s2 <= length s1 + (length s2 + length s2 )).  lia. clear H.
  apply add_le_cases' in H0. inversion H0.
  + inversion H. 
            destruct IH1 as [x [ x0 [ x1[H3[H4[ H5 H6]]]]]]. apply H. exists x, x0, (x1++s2).
            split. rewrite  H3. rewrite <- app_assoc. rewrite <- app_assoc. auto.
            split. auto. split. auto. intros.
            rewrite app_assoc. rewrite app_assoc. remember ((x ++ napp m x0) ++ x1) as s.
            apply MStar_plus. rewrite Heqs. rewrite <- app_assoc. apply H6. auto.
  + clear H0. inversion H. inversion H0.
    *  destruct s1. simpl. simpl in H1.  simpl in H'. simpl in H2. 
            destruct IH2 as [x [ x0 [ x1[H3[H4[ H5 H6]]]]]]. simpl in H2. simpl. apply H'.
             simpl. exists x, x0, (x1). split. rewrite  H3. auto. split. auto. split. auto. 
            intros. remember ((x ++ napp m x0) ++ x1) as s1'. rewrite app_assoc. rewrite <- app_assoc. apply H6.
    ++ exists [], (x::s1), s2. split. auto. split. intros H4. inversion H4. split. 
       assert(forall a b , b > a -> a <=b) . lia. apply H3 in H1. auto. remember (x::s1) as s'.
      intros. induction m. simpl. auto. simpl. rewrite <- app_assoc. simpl in IHm. 
      remember (napp m s' ++ s2) as s'''. auto. apply MStar_plus. auto. auto.
  * inversion H0. assert (forall a b, a > b + a -> 0 > b). lia. apply H3 in H2. inversion H2.
...
Proof.
intros. split.
  - intro H. inversion H. destruct s1 as [| a' s1'] .
    + left. split.
      * apply H3.
      * simpl. apply H4.
    + right. inversion H1. exists s1', s2. split.
      * auto.
...
Proof.
  ...
  - remember (Star re) as re'.
    ...
    + intros. destruct s1.
      * simpl in Heqs'. auto.    
      * exists s1, s2. inversion Heqre'. auto. subst.
        inversion Heqs'. split. simpl. reflexivity. split. auto. subst. apply H. subst. apply H0.      
  - intros [s0 [s1 [H1 [H2 H3]]]].
    rewrite H1. eapply (MStarApp (a :: s0) s1). apply H2. apply H3. 
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. auto. constructor. intros H'. inversion H'.
  - simpl. apply ReflectT. constructor. 
  - simpl. auto. apply ReflectF. unfold not. intros H'. inversion H'.
  - simpl. auto. inversion IHreg_exp1. simpl. inversion IHreg_exp2.
    + simpl. apply ReflectT. unfold not. eapply (MApp [] _ []); assumption.
    +  simpl. apply ReflectF. intro H3'.
      inversion H3'. subst. destruct s1 as [| h1 t1].
      * destruct s2 as [| h2 t2]. auto. inversion H5.
      *  inversion H5.
    + simpl. auto. constructor. auto. intros H1'. inversion H1'. auto. subst. destruct s1 as [| a b]. auto. inversion H3. 
  -  inversion IHreg_exp1. + inversion IHreg_exp2. 
      * simpl. apply ReflectT. auto. apply MUnionL. auto.
      * simpl. apply ReflectT. auto. apply MUnionL. auto.
       + inversion IHreg_exp2. 
      * auto. simpl. apply ReflectT. auto. apply MUnionR. auto.
      * simpl. apply ReflectF. auto. unfold not. intro a. auto. inversion a. auto. contradiction. 
   - simpl. inversion IHreg_exp. 
    + apply ReflectT. inversion H0. auto. auto.  apply MStar0.
    + apply ReflectT. inversion H0. auto. auto. apply MStar0.     
Qed.
(** [] *)
...
match re with
  ...
  | App r1 r2 => if match_eps r1 then Union (App (derive a r1) r2) (derive a r2)
...
Proof.
 ...
 auto. unfold derives. unfold is_der. split.
  - generalize dependent a. generalize dependent s. induction re.
    + intros; auto; inversion H.
    + intros; auto; inversion H.
    + intros . simpl. inversion H.  rewrite eqb_refl. auto ; apply MEmpty.
    + simpl. auto.  destruct (match_eps_refl re1).
      * intros. apply union_disj.
        apply app_ne in H0. destruct H0 as [[H1 H2] | [s' [s'' [P' [Q' R]]]]].
        apply IHre2 in H2. right. auto. apply IHre1 in Q'. left. rewrite P'. auto. apply MApp. auto... auto.
      * intros. apply app_ne in H0. destruct H0 as [[H1 H2] | [s' [s'' [P' [Q' R]]]]].
        apply H in H1. destruct H1. auto. rewrite P'.  apply MApp. apply IHre1 in Q'. auto. auto.   
    +  intros. apply union_disj. apply union_disj in H. destruct H .
      * apply IHre1 in H. left. auto. * apply IHre2 in H. right. auto.
    +  intros. apply star_ne in H. destruct H as  [s1 [s2 [P [Q R]]]]. auto. try rewrite P. apply MApp.
      * apply IHre in Q. auto. *  auto. 
  - generalize dependent a. generalize dependent s. induction re. subst.
    + intros; auto; inversion H. + intros; auto; inversion H. + simpl . intros . destruct (eqb a t) eqn:E.
      * inversion H. apply eqb_eq in E. rewrite E. auto. apply MChar. * auto. try inversion H.
    +  simpl in *.  destruct (match_eps_refl re1). 
      * intros. apply union_disj in H0. destruct H0. inversion H0.
        ** apply IHre1 in H4. replace (a :: s1 ++ s2) with ((a :: s1) ++ s2). apply MApp. apply H4. apply H5.  auto.
        ** simpl. auto. apply IHre2 in H0. auto.  apply (MApp nil re1 (a :: s) re2). auto. auto.
       * intros. simpl in H. unfold not in H. inversion H0. 
          replace (a :: s1 ++ s2) with ((a :: s1) ++ s2). apply MApp. apply IHre1 in H4. auto. auto. auto.
    + intros. simpl. apply union_disj . apply union_disj in H.
      destruct H as [ H1 | H2]. simpl. apply IHre1 in H1.  auto. apply IHre2 in H2. auto.
    + intros. inversion H. replace (a :: s1 ++ s2) with ((a :: s1) ++ s2).
      apply MStarApp. auto. auto. auto.
Qed.
...
(** **** Exercise: 3 stars, standard (regex_refl) 
...
Proof.
  unfold matches_regex. intros s re.
  ...
  - intros. apply iff_reflect. split.
    + simpl. intros H. apply derive_corr  in H. auto. 
    assert ( x :: s =~ re <-> s =~ derive x re). auto. 
      * apply derive_corr.
      * simpl. assert (reflect (s =~ (derive x re)) (regex_match s (derive x re))).
        apply IHs. simpl in  H1. specialize (IHs (derive x re)).
        apply reflect_iff in IHs. auto. apply IHs. auto. 
    +  intros. apply (derive_corr x re). specialize (IHs (derive x re)).

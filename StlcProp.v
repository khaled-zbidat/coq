...
Proof.
  ...
  intros t.
  induction t; intros T T'; auto.
    - solve_by_inverts 2.
    - right. inversion T';subst. specialize IHt1 with <{T2 -> T}> as [h1 | h1];
      specialize IHt2 with T2 as [h2 | h2];subst;auto.
      + inversion h1;subst;try solve_by_invert. exists <{[x0:=t2] t0}>;auto.
      + inversion h2;subst. exists <{t1 x0}>;auto.
      + inversion h1;subst. exists <{x0 t2}>;auto.
      + inversion h1;subst. exists <{x0 t2}>;auto. 
    - right. inversion T'. subst. apply IHt1 in H3. inversion H3.
      + inversion H. subst. solve_by_inverts 2.
      simpl.
        * exists t2. apply ST_IfTrue.
        * exists t3. apply ST_IfFalse.
      + inversion H as [t1' Ht1]. exists (tm_if t1' t2 t3). apply ST_If. apply Ht1.
Qed.
(** [] *)
...
Proof.
  ...
  destruct (eqb_stringP x x0);subst.
    - rewrite update_eq in H. injection H as H. simpl. subst. apply weakening_empty. apply Hv.
    - apply T_Var. rewrite update_neq in H. auto. apply n.
  - destruct (eqb_string x x0) eqn:eq.
    + apply eqb_string_true_iff in eq;subst. rewrite update_shadow in Ht. auto.
    + apply eqb_string_false_iff in eq;subst. constructor. apply IHHt. apply update_permute. assumption.
Qed.
...
(** **** Exercise: 2 stars, standard (subject_expansion_stlc) 
...
no it does not hold for stlc
for example we can build this lemma 
Lemma subject_expansion_dosent_wrok_in_stlc : exists t t' T,
  \empty |- t' \in T /\ t ==> t' /\ ~(\empty |- t \in T).
and this can be proved as a counter example.
...
Lemma subject_expansion_dosent_wrok_in_stlc: exists t t' T,
  empty |- t' \in T /\ t --> t' /\ ~(empty |- t \in T).
Proof.
 exists (tm_app (tm_abs x (Ty_Arrow Ty_Bool Ty_Bool) tm_true) tm_true).
 exists tm_true.
 exists Ty_Bool.
 split. apply T_True. split. apply ST_AppAbs. auto.  unfold not. intros. solve_by_inverts 3.
Qed.
...
Theorem unique_types : forall Gamma e T T',
  ...
  T = T'.
Proof.
intros Gamma t T T' HT. generalize dependent T'. induction HT.
  - intros. inversion H0. subst. rewrite H in H3. inversion H3.  auto.
  - intros. inversion H. subst. apply IHHT in H5. subst. auto.
  - intros. inversion H. subst. apply IHHT1 in H3. inversion H3. auto.
  - intros. inversion H. auto. 
  - intros. inversion H. auto.
  - intros. inversion H. subst. apply IHHT2 in H6. apply H6.
Qed.
(** [] *)
...
Proof.
  ...
  inversion H1;subst. apply IHappears_free_in in H7. rewrite update_neq in H7. auto. auto.
Qed.
(** [] *)
...
and the following typing rule:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
(* FILL IN HERE *)
        it will become false i will give a counter example: 
        tm_if tm_true tm_false tm_false --> tm_false \/ tm_if tm_true tm_false tm_false --> zap
      - Progress
(* FILL IN HERE *)
        it will become false - it wont hold because zap is not a value where it will not stop.
      - Preservation
(* FILL IN HERE *)
        it will remain true.
*)
...
(* FILL IN HERE *)
    it will remain true.
      - Preservation
(* FILL IN HERE *)
    becomes false - it wont hold because becayse foo has no type.
*)
...
(* FILL IN HERE *)
      ...
      - Progress
(* FILL IN HERE *)
    it will be false for example (tm_if0 tm_true then (x:BOOL,x) else  (x:BOOL, X) tm_true) only ST_App1
    will allow us to step sice its a value.
...
(** **** Exercise: 5 stars, standard, optional (stlc_arith) 

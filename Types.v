...
Proof.
    ...
    inversion H; intros; inversion H1; inversion H2. induction H.
...
Proof.
  ...
  - (* T_Test *)
    ...
    + left.
      unfold value.
      right. apply nv_scc. inversion H.
      * inversion H0; subst; inversion HT.
      * assumption.
    + right.
      inversion H. exists (scc x)... auto.
    - right.
      inversion IHHT.
      + inversion H.
        * inversion H0; subst; inversion HT.
        * inversion H0.
          exists zro... auto. exists t... auto.
      + inversion H.
        exists (prd x)... auto.
    - right.
      inversion IHHT.
...
Proof with eauto.
 ...
 (induction HE) ; intros T HT; inversion HT; subst; clear HT; auto.
...
(** **** Exercise: 2 stars, standard (subject_expansion) 
    ...
    subject_expasion is wrong :
      here is an example

    (test tru zro tru ==> zro) /\
    (|- zro \in TNat) /\
    ~ (|- test tru zro trut \in TNat).
...
(** **** Exercise: 2 stars, standard (variation2) 
   ...
   this rule?  For each one that does, give a counter-example.
    ...
    - Determinism of [step]:
        becomes false
          test tru tru fls --> tru /\
          test tru tru fls --> fls
    - Progress
        remains true
    - Preservation
            remains true
...
(** **** Exercise: 2 stars, standard, optional (variation3) 
   ...
   this rule?  For each one that does, give a counter-example.
    ...
    - Determinism of [step]:
        becomes false
          test tru (prd (scc zro)) zro --> prd (scc zro) /\
          test tru (prd (scc zro)) zro --> test tru zro zro
    - Progress
        remains true
    - Preservation
            remains true
...
(** **** Exercise: 1 star, standard, optional (remove_prdzro) 
